#include "VRD3D12.h"
/// <summary>
/// Back Buffer Index. this should be configurable.
/// </summary>
...
void VRD3D12::CreateContexts(HWND win)
{
	...
	for (UINT a = 0; a < Buffers; a++)
	{
		...
		/// Push Resources to ResourcePool for destruction.
		ResourcePool.push_back(RT[a].Get());
	}
	BBIndex = Swap->GetCurrentBackBufferIndex();
...
void VRD3D12::DestroyResources()
{
	if (Queue)
		WaitForPrevFrame();

	for (auto& r : ResourcePool)
	{
		r->Release();
	}
}
...
void VRD3D12::FindAdaptors()
{
	...
	if (ta == nullptr) {
		throw "No compatible adapter found.";
	}
}
...
{
	const UINT64 fence = (UINT64)FenceValue;
